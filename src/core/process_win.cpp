...
Process::id_type Process::open(const std::string &command, const std::string &path) {
  ...
  BOOL bSuccess = CreateProcessA(NULL, command_cstr, NULL, NULL, TRUE, 0,
...
void Process::async_read() {
  ...
  if(stdout_fd) {
    stdout_thread=std::thread([this](){
      ...
      for (;;) {
        BOOL bSuccess = ReadFile(*stdout_fd, static_cast<CHAR*>(buffer.data()), static_cast<DWORD>(buffer_size), &n, NULL);
        if(!bSuccess || n == 0)
          break;
  ...
  if(stderr_fd) {
    stderr_thread=std::thread([this](){
	  ...
	  std::vector<char> buffer(buffer_size);
      for (;;) {
        BOOL bSuccess = ReadFile(*stderr_fd, static_cast<CHAR*>(buffer.data()), static_cast<DWORD>(buffer_size), &n, NULL);
        if(!bSuccess || n == 0)
          break;
